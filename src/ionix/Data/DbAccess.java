...
public class DbAccess implements AutoCloseable {
    ...
    protected PreparedStatement prepareStatement(SqlQuery query, AutoGeneratedKey autoGeneratedKey, String columnName) {//columnName Oracle Squence returns key için eklendi.
        if (null == query)
        ...
        try {
            String sql = impl.getQuery();
            if (null != autoGeneratedKey)
                ps = this.conn.prepareStatement(sql, autoGeneratedKey.getValue());
             else if (null != columnName)
                ps = this.conn.prepareStatement(sql, new String[]{columnName});
            else
    ...
    public <T> T executeScalar(Class<T> cls, SqlQuery query){
        return (T)Convert.Instance.changeType(this.executeScalar(query), cls);
    }
    ...
    public <T> Iterable<T> executeScalarList(Class<T> cls, SqlQuery query) {
        ...
        for(Object item : it){
            list.add((T) Convert.Instance.changeType(item, cls));
        }
        return list;
    }
